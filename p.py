...
def insertTweets(connection, tweet, id_user, id_region):
    try:
        x = connection.cursor()
        x.execute('INSERT INTO Tweet VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ', (
            tweet.id, tweet.created_at, cleanTweet(tweet.text.encode("utf-8")).encode('utf-8'),
...
def main():
    cn = getConnection()
    get_all_tweets(126471512, "Tarapaca")
    # print countTweets(cn, 126471512)
